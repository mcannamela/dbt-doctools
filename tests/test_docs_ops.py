...
from dbt_doctools.docs_ops import (
    ...
    compute_min_doc_depth,
    make_doc_sort_fun,
    is_non_empty,
    find_consolidated_docs_and_duplicates,
    find_degenerate_docs,
...
def test_consolidate_duplicate_docs_blocks_(manifest, graph, config):
    ...
    lack_of_degenerate_docs = find_degenerate_docs(
        non_empty_docs(manifest_without_dupes)
    )
...
def test_find_consolidated_docs_and_duplicates(config, graph, manifest):
    ...
    doc_id_to_dupe_ids = {
        t[0].unique_id: [u.unique_id for u in t[1]]
        for t in consolidated_docs_and_duplicates
    }
    project = config.project_name
    assert doc_id_to_dupe_ids[f"{project}.zero_layer__old_value"] == [
        f"{project}.one_layer__old_value"
    ]
...
def test_make_doc_sort_fun(manifest, graph, config):
    ...
    assert sorted_docs.index(f"{project}.static_map_value") < sorted_docs.index(
        f"{project}.zero_layer__old_value"
    )
    assert sorted_docs.index(f"{project}.zero_layer__old_value") < sorted_docs.index(
        f"{project}.one_layer__old_value"
    )
...
def test_compute_min_doc_depth():
    ...
    exp_block_depth = {"ka": 0, "kb": 1, "kc": 2, "kd1": 2, "kd2": 3, "ke": 4}
