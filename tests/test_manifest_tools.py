...
def test_unsafe_inferred_id_type_for_models_and_seeds(manifest):
    for node_id, node in manifest.nodes.items():
        if node.resource_type in {NodeType.Model, NodeType.Seed}:
            assert unsafe_inferred_id_type(node_id) == node.resource_type
        else:
            with pytest.raises(NotImplementedError):
                unsafe_inferred_id_type(node_id)

def test_unsafe_inferred_id_type_for_sources(manifest):
    for source_id, source in manifest.sources.items():
        assert unsafe_inferred_id_type(source_id) == NodeType.Source
